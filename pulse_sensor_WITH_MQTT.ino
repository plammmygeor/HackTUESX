...
const char* sensor1_topic = "HACKTUESX/QUATRO/sens";
...
////////// Sensor
  ...
  Time = millis();
  int summer = Time % 15000;
  int level = analogRead(13);
  //Serial.println(level);
  //Serial.println(summer);
  if(level != 0 & level > 600 & set1 == 1 & set2 == 1)
  {
    set1 = 0;
    set2 = 0;
    counter++;
  ...
  String payload = String(pulse); // Generate a random number between 0 and 100
  
  // Publish random data to MQTT broker
  client.publish(sensor1_topic, payload.c_str(), true);
